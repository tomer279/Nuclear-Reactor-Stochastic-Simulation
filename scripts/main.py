...
from src.models.core_parameters import (
    ...
    DeadTimeParameters,
    FissionParameters)
from src.services import simulation_runner as sim_run
...
# =============================================================================
# CORE SIMULATION PARAMETERS - MODIFY THESE FIRST
# =============================================================================
# These parameters control the fundamental simulation behavior
# and auto-generate file naming prefixes

# Time discretization
SIMULATION_STEPS = 100_000_000        # Number of time steps for stochastic
GRID_POINTS = 1_000_000             # Number of grid points for numerical methods
T_START = 0.0                       # Simulation start time (seconds)
T_END = 0.1                         # Simulation end time (seconds)

# Dead time parameters
MEAN_TAU = 1e-6                     # Mean dead time (seconds)
# Standard deviation of dead time (seconds)
STD_TAU = 0.2 * MEAN_TAU
# Distribution type: 'constant', 'uniform', 'normal', 'gamma'
DEAD_TIME_DISTRIBUTION = 'uniform'
...
RUN_RUNGE_KUTTA = True      # Run Runge-Kutta method simulations
RUN_CPS_ANALYSIS = True      # Run count rate analysis and plot comparison

# =============================================================================
# AUTO-GENERATED PREFIXES - DO NOT MODIFY
# =============================================================================
# These are automatically computed from the parameters above

# Stochastic uses custom prefix (could be auto-generated if desired)
STOCHASTIC_PREFIX = 'hunmil_f'

# Numerical methods: auto-generate full descriptive prefix
_GRID_PREFIX = utl.generate_prefix_from_steps(
    GRID_POINTS, simulation_type='numerical')
_MEAN_STR = f"mean{MEAN_TAU:.0e}"
_STD_STR = f"std{STD_TAU:.0e}"

# Full prefix includes grid size and dead time parameters
if DEAD_TIME_DISTRIBUTION == 'constant':
    NUMERICAL_PREFIX = f"{_GRID_PREFIX}_{_MEAN_STR}"
else:
    NUMERICAL_PREFIX = f"{_GRID_PREFIX}_{_MEAN_STR}_{_STD_STR}"

# Use same prefix for all numerical methods
EM_PREFIX = NUMERICAL_PREFIX
TAYLOR_PREFIX = NUMERICAL_PREFIX
RK_PREFIX = NUMERICAL_PREFIX

print(f"Using numerical method prefix: {NUMERICAL_PREFIX}")
...
# =============================================================================
# Control which dead time type to analyze (should match what you ran above)
ANALYZE_DEAD_TIME_TYPE = DEAD_TIME_DISTRIBUTION  # Auto-match the simulation
...
class SimulationManager:
    ...
    def __init__(self):
        """Initialize simulation manager with global parameters."""

        self.physical_params = PhysicalParameters()

        # Create time parameters from global constants
        ...
        # Create dead time parameters from global constants
        self.dead_time_params = DeadTimeParameters(
            mean_tau=MEAN_TAU,
            tau_distribution=DEAD_TIME_DISTRIBUTION,
            std_tau=STD_TAU
        )
    ...
    def _calculate_theoretical_cps(self):
        ...
        theoretical_cps = cr.calculate_theoretical_cps_for_fission_rates(
            ...
            std_tau_s=self.dead_time_params.get_std(),
