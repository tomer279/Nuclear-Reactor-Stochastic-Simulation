...
def calculate_theoretical_cps(
    ...
    detection_rate = detect * equil

    calculator = DeadTimeLaplaceCalculator()

    std = None if std_tau_s == 0.0 else std_tau_s

    laplace_result = calculator.calculate_laplace_transform(
        x=detection_rate,
        tau=mean_tau_s,
        distribution=dead_time_type.lower(),
        std=std)

    theoretical_cps = detection_rate * laplace_result.value
