...
def extract_simulation_prefix(index: str, fission_value: float) -> str:
    ...
    """
    # Try to find fission value in the index with different formats
    fission_patterns = [
        f"_f{fission_value:.2f}",  # matches '_f33.95'
        f"_f{fission_value:.3f}",  # matches '_f33.950'
        f"{fission_value:.2f}",    # matches '33.95'
        f"{fission_value:.3f}",    # matches '33.950'
        str(fission_value)         # matches exact string
    ]

    for pattern in fission_patterns:
        if pattern in index:
            parts = index.split(pattern)
            return parts[0]

    # No fission found in index, return unchanged
    return index
...
def generate_filename(
    ...
    >>> # Distribution-based prefix
    >>> filename = generate_filename(
    ...     'uniform_tau1.00e-06_std1.00e-07', 'EM', 'uniform', 33.95
    ... )
    >>> print(filename)  
    >>> # 'EM_uniform_Dead_Time_uniform_tau1.00e-06_std1.00e-07_f33.950.npy'
    """

    fission_str = f"{fission_value:.3f}"

    # Check if prefix ends with common patterns that shouldn't have separator
    if prefix.endswith('f') or prefix.endswith('_'):
        # Prefix ends with 'f' or '_', append fission directly
        return (f"{method}_{dead_time_type}_Dead_Time_"
                f"{prefix}{fission_str}{extension}")

    # Add separator before fission
    return (f"{method}_{dead_time_type}_Dead_Time_"
            f"{prefix}_f{fission_str}{extension}")


def generate_prefix_from_steps(
        steps: int,
        simulation_type: str = 'numerical'
) -> str:
    """
    Automatically generate descriptive prefix from number of steps/grid points.

    This function converts step counts into readable prefixes using scientific
    notation or common shorthand (e.g., 1M, 100M, etc.).

    Parameters
    ----------
    steps : int
        Number of simulation steps or grid points
    simulation_type : str, optional
        Type of simulation ('numerical' or 'stochastic'), by default 'numerical'

    Returns
    -------
    str
        Descriptive prefix for file naming

    Examples
    --------
    >>> prefix = generate_prefix_from_steps(1_000_000)
    >>> print(prefix)  # '1e6gp' (1e6 grid points)

    >>> prefix = generate_prefix_from_steps(100_000_000)
    >>> print(prefix)  # '1e8gp'

    >>> prefix = generate_prefix_from_steps(5_000_000)
    >>> print(prefix)  # '5e6gp'

    >>> prefix = generate_prefix_from_steps(1_000_000, 'stochastic')
    >>> print(prefix)  # '1e6steps'

    Notes
    -----
    The function uses scientific notation for clarity and consistency.
    For numerical methods, uses 'gp' (grid points) suffix.
    For stochastic, uses 'steps' suffix.
    """
    if steps == 0:
        return '0'

    # Find the exponent in scientific notation
    exponent = int(np.floor(np.log10(steps)))
    mantissa = steps / (10 ** exponent)

    if abs(mantissa - round(mantissa)) < 0.01:
        mantissa = int(round(mantissa))

    suffix = 'steps' if simulation_type == 'stochastic' else 'gp'

    if mantissa == 1:
        return f"1e{exponent}{suffix}"

    return f"{mantissa}e{exponent}{suffix}"
