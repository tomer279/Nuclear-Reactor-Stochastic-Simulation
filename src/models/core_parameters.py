...
import numpy as np
from src.utils.utils import mean, variance
...
class DeadTimeParameters:
    ...
    types including constant, normal, uniform, and gamma.
    ...
    std_tau : float
        Standard deviation of dead time in seconds (0 for constant distribution)
    ...
    def __init__(
        ...
        if (self.std_tau is None
                and self.tau_distribution.lower() != 'constant'):
            raise ValueError(
                f"No standard deviation determined for {self.tau_distribution}")

        if tau_params is None:

            self.tau_params = self._generate_distribution_params()
        else:
            self.tau_params = tau_params

    def _generate_distribution_params(self):
        """
        Auto-generate distribution-specific parameters 
        from mean_tau and std_tau.

        Notes
        -----
        For constant distribution, std_tau is not used.
        For other distributions, std_tau must be provided during initialization.
        """
        params = {
            'constant': {
                'tau': self.mean_tau
        ...
        if self.std_tau is not None:
            ...
            params['uniform'] = {
                'low': self.mean_tau - np.sqrt(3) * self.std_tau,
                'high': self.mean_tau + np.sqrt(3) * self.std_tau
            }
            params['gamma'] = {
                'shape': (self.mean_tau / self.std_tau) ** 2,
                'scale': (self.std_tau ** 2) / self.mean_tau
            }
        return params
    ...
    def get_std(self) -> float:
        """
        Get the standard deviation of the current dead time distribution.

        Returns
        -------
        float
            Standard deviation of the dead time distribution in seconds.
            Returns 0.0 for constant distribution or when std_tau is None.
