...
A comprehensive Streamlit application
for simulating neutron population dynamics in fission chains
with dead time analysis capabilities.
...
import numpy as np
from src.visualization.plot_simulations import plot_stochastic_population_dynamics
from src.models.models import (SimulationParameters,
                    DeadTimeAnalysis,
                    AppState)
from src.services.services import SimulationService, DeadTimeAnalysisService
from src.ui.ui_components import ParameterInput, ResultsDisplay, DeadTimeAnalysisUI
...
def calculate_detection_stats(detection_matrix):
    ...
    simulation_time = detection_matrix[0][-1] if not np.isnan(
        detection_matrix[0][-1]) else 0
...
def _run_dead_time_analysis(results_dict, params, dead_time_config):
    """
    Run dead time analysis on simulation results.

    Executes dead time analysis using the provided configuration and stores
    ...
    Raises
    ------
    ValueError: If dead time configuration is invalid
    ...
    try:
        ...
        st.success(f"Dead time analysis {dead_time_config.analysis_name} "
                   "completed!")

    except ValueError as e:
        st.error(f"Invalid dead time parameters: {str(e)}")
...
def _add_theoretical_cps_to_analysis(analysis_name, params):
    ...
    try:
        updated_analysis = dead_time_service.add_theoretical_cps(
            analysis, params)
        ...
        st.error(f"Invalid parameters for theoretical CPS: {str(e)}")
    except RuntimeError as e:
        st.error(f"Theoretical CPS calculation failed: {str(e)}")
...
def _run_simulations_for_selected_rates(params: SimulationParameters):
    ...
    Exception: If simulation fails,
        displays error message and exception details
    """
    try:
        ...
        st.success(
            f"Stochastic simulations completed for {len(params.fission_rates)}"
            f"fission rates!")
...
def _run_all_fission_rate_simulations(params, progress_bar, status_text):
    ...
    for i, fission_rate in enumerate(params.fission_rates):
        ...
        status_text.text(
            f"Simulating fission rate {fission_rate} ({i+1}/{total_rates})...")
        ...
        single_result = simulation_service.run_multiple_simulations(
            single_rate_params)
...
def _display_simulation_tabs(results, results_display):
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Comparative Analysis", "Individual Plots", "Summary Table"])
...
def _display_first_dead_time_analysis(results, params):
    ...
    continue_analysis = st.button(
        "Continue with Dead Time Analysis", type="secondary")

    if continue_analysis:
        _run_dead_time_analysis(results, params, config)
        st.rerun()
...
def _display_existing_dead_time_analyses(results, params):
    ...
    app_state = st.session_state.app_state
    st.success(
        f"Dead time analysis completed!"
        f"({len(app_state.dead_time_analyses)} analyses)")

    dead_time_ui = DeadTimeAnalysisUI()

    # Display all analyses
    tab_dt1, tab_dt2, tab_dt3 = st.tabs(
        ["CPS Comparison", "Individual Analyses", "Add New Analysis"])
...
def _handle_new_analysis_creation(config, results, params):
    ...
    if app_state.get_analysis_by_name(config.analysis_name):
        st.error("Analysis name already exists!"
...
def _display_welcome_header():
    ...
    population dynamics in fission chains
    with comprehensive **dead time analysis**.
    ...
    - **Dead Time Analysis**: Count rate analysis with
    multiple dead time distributions
    - **Theoretical CPS**: Analytical count rate calculations
    (equilibrium only)
    ...
    - **Available only with equilibrium values**: Theoretical formulas assume
    steady-state conditions
    - **Multiple distributions**: Constant, Normal, Uniform, and Gamma
    dead time models
    - **Analytical formulas**: Based on stochastic differential equations
    and dead time statistics
    - **Comparative analysis**: Side-by-side comparison of simulated
...
def _display_initial_population_info(params):
    ...
    if params.use_equilibrium:
        st.write(
            "**Initial Population:** Equilibrium values"
            "(enables theoretical CPS)")
    else:
        st.write(
            f"**Initial Population:** {params.initial_population}"
            f"(theoretical CPS not available)")
...
def _display_dead_time_preview():
    ...
    - Configure dead time parameters
    ...
    - Compare different dead time distributions
    (Constant, Normal, Uniform, Gamma)
...
def _display_theoretical_cps_info(params: SimulationParameters):
    ...
    if params.use_equilibrium:
        st.success("""
        ✅ **Theoretical CPS Available**: You're using equilibrium values,
        so theoretical CPS calculations will be available
        after running dead time analysis.
        ...
        **What you can do:**
        ...
        st.warning("""
        ⚠️ **Theoretical CPS Not Available**:
            You're using a fixed initial population,
            so theoretical CPS calculations won't be available.
